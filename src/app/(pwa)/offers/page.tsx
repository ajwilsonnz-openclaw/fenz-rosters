...
export default function FirefighterOffersPage() {
    ...
    const fetchMyOffers = async () => {
        ...
        // For testing purposes, if you aren't using Supabase Auth yet, 
        // you can hardcode an email here to see the UI working:
        const userEmail = session?.user?.email || "luke.tanner@fenz.slack.com"; // Replace with actual session email

        // 2. Find their firefighter ID
        const { data: ff } = await supabase.from('firefighters').select('id').eq('email', userEmail).single();

        if (ff) {
            // 3. Fetch their pending and recently responded offers
    ...
    return (
        <main className="min-h-screen bg-slate-50 pb-20">
            ...
            <header className="bg-primary text-white p-6 rounded-b-3xl shadow-sm">
                <h1 className="text-2xl font-black tracking-tight">My Shifts</h1>
                <p className="text-primary-foreground/80 text-sm font-medium mt-1">Review and manage your overtime offers.</p>
            </header>
                                ...
                                return (
                                    <Card key={offer.id} className="p-5 rounded-2xl border-blue-100 shadow-md ring-1 ring-blue-50 bg-white">
                                        ...
                                        <div className="flex gap-3">
                                            <Button onClick={() => openDeclineDialog(offer)} variant="outline" className="flex-1 rounded-xl border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700 font-bold">
                                                Decline
                                            </Button>
                                            <Button onClick={() => handleAccept(offer.id)} disabled={isSubmitting} className="flex-1 rounded-xl bg-green-600 hover:bg-green-500 text-white font-bold shadow-sm">
